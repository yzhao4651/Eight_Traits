...
###find the different both times 

flo.20 <- read.csv("Flotraits/flounique3.csv")
str(flo.20)
##select the MAF < 0.01
...
gene_df.genes <- Findneargenes(flo.20, 1e4)
str(gene_df.genes)
...
gene_df.chr <- Findneargenes(flo.20, 1e4)
str(gene_df.chr)
...
gene_df.start <- Findneargenes(flo.20, 1e4)
str(gene_df.start)
...
gene_df.end <- Findneargenes(flo.20, 1e4)
str(gene_df.end)
Findneargenes <- function(mySNPs, search_radius){
  saved_genes <- list()
  length(saved_genes) <- nrow(mySNPs)
...
gene_df.width <- Findneargenes(flo.20, 1e4)
str(gene_df.width)
...
gene_df.strand <- Findneargenes(flo.20, 1e4)
str(gene_df.strand)
...
nlevels(gene_df8.4.a$SNP) ##1033
nlevels(gene_df8.4.a$Gene)###4122

write.csv(gene_df8.4.a,file="Flotraits2/AHG.Flotraits2.T.hitandnohit.csv")

##merge all of data withunique

flo.8.4.T <- merge(flo.20, gene_df8.4.a,by="SNP",all.x=TRUE)
flo.8.4.T <- droplevels(flo.8.4.T)
nlevels(flo.20$SNP)##1675
nlevels(flo.8.4.T$Gene)##4122
nlevels(flo.8.4.T$SNP)###1675
###get all of the no any hit SNP
flo.8.4.T.nohhit <- flo.8.4.T[is.na(flo.8.4.T$Gene),]
flo.8.4.T.nohhit <- droplevels(flo.8.4.T.nohhit)
nlevels(flo.8.4.T.nohhit$SNP)###642
write.csv(flo.8.4.T.nohhit,file="Flotraits2/AHG.Flotraits2.T.nohit.csv")
...
nlevels(flo.8.4.T.hit$SNP) ###1033
nlevels(flo.8.4.T.hit$Gene)###4122

write.csv(flo.8.4.T.hit,file="Flotraits2/AHG.Flotraits2.T.hit.csv")
...
nlevels(as.factor(AHG.flo.5$SNP)) ###1033
nlevels(as.factor(AHG.flo.5$Gene))###2946

write.csv(AHG.flo.5,file="Flotraits2/AHG.Flotraits2.anota.csv")
###select the Genes without studying: which means the X.pac is NA 
flo.hit.gene <- read.csv("Flotraits2/AHG.Flotraits2.anota.csv")
str(flo.hit.gene)
nlevels(as.factor(flo.hit.gene$Gene))#2942
...
nlevels(flo.NOX.pacId$SNP)###130
nlevels(flo.NOX.pacId$Gene)###366
write.csv(flo.NOX.pacId,file="Flotraits2/AHG.flo.NOX.pacId.csv")

## with some hit 
flo.X.pacId <- flo.hit.gene[!is.na(flo.hit.gene$X.pacId),]
flo.X.pacId <- droplevels(flo.X.pacId)
nlevels(flo.X.pacId$SNP)###903
nlevels(flo.X.pacId$Gene)###2580
write.csv(flo.X.pacId,file="Flotraits2/AHG.flo.X.pacId.csv")

###check how many hit with ara 
flo.NO.ara <- flo.X.pacId[flo.X.pacId$Best.hit.arabi.name=="",]
flo.NO.ara <- droplevels(flo.NO.ara)
names(flo.NO.ara)
nlevels(flo.NO.ara$SNP)###368
nlevels(flo.NO.ara$Gene)###473
write.csv(flo.NO.ara,file="Flotraits2/AHG.flo.X.pacId.NO.ara.csv")
...
nlevels(flo.X.pacId.ara$SNP)###872
nlevels(flo.X.pacId.ara$Gene)###2107
write.csv(flo.X.pacId.ara,file="Flotraits2/AHG.flo.X.pacId.ara.csv")
...
nlevels(flo.X.pacId.GO$SNP)###741
nlevels(flo.X.pacId.GO$Gene)###1275
write.csv(flo.X.pacId.GO,file="Flotraits2/AHG.flo.X.pacI.GO.csv")
#### rice 
...
nlevels(flo.X.pacId.rice$SNP)###891
nlevels(flo.X.pacId.rice$Gene)###2263
write.csv(flo.X.pacId.rice,file="Flotraits2/AHG.flo.X.pacI.rice.csv")
...
write.csv(AHG.flo.5.ara.hit.2, file="Flotraits2/AHG.flo.5.ara.hit.2.csv")
...
write.csv(AHG.flo.hit.T, file="Flotraits2/hitflowithotherspecies.csv")

##find the genes from the miscanthus 
