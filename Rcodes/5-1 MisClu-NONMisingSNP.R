...
myGDC.125.2562 <- read.csv("data/myGDC.125.2562.csv",row.names = 1)
row.names(myGDC.125.2562)
myGDC.125.2562 <- myGDC.125.2562[-c(121),]
source("Function/SelectMAF-GAPIT.R")
myGD.124.2560 <- Select.MAF(myGDC.125.2562)
...
write.csv(myGM.124.2560, file = "data/myGMC.124.2560.csv",row.names = FALSE)
source("Function/FirstColumn.R")
myGD.124.2560 <- FirstColumn(myGD.124.2560)
write.csv(myGD.124.2560, file = "data/myGDC.124.2560.csv",row.names = FALSE)


##############step5 myY: Phenotype data for GAPIT and FarmCPU 
##############step5 myY: Phenotype data for GAPIT and FarmCPU 
##import the dataset with myY.125.2562 
myYC.125.2562 <- read.csv("data/myYC.125.2562.csv")
myY.124.2560 <- myYC.125.2562[match(myGD.124.2560$Taxa,myYC.125.2562$Taxa, nomatch=0),]
myY.124.2560 <- droplevels(myY.124.2560)
######### order the Taxa
#myYorderg <- na.omit(myYorderg[order(myYorderg$Taxa),])

#### write out the phenotype with correct TAXA
write.csv(myY.124.2560, file = "data/myYC.124.2560.csv", row.names = FALSE, na = "")
...
#str(myQ.clum.107.3707)
####write out the dataset
write.csv(myQ.124.2560, file = "data/myQC.124.2560.csv",row.names = FALSE)
